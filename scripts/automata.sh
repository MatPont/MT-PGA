...
for isPD in 0 1; do
  for j in `seq 1 $noRun`; do
    ...
    for i in `seq 0 $len`; do
      ...
      echo -e "${GREEN}_____________ gpcaMT.py $path $pt $eps $eps2 $eps3 $coef $barySizeLimit $isPD 0 _____________${NC}"
      pvpython gpcaMT.py $path $pt $eps $eps2 $eps3 $coef $barySizeLimit $isPD 0
    done
  done
done
