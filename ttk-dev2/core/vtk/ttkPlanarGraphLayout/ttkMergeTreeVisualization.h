...
public:
  ...
  void copyPointData(vtkUnstructuredGrid *treeNodes,
    ...
    if(not treeNodeIdArray) {
      for(int i = 0; i < treeNodes->GetPointData()->GetNumberOfArrays(); ++i) {
        auto dataArray
          = vtkDataArray::SafeDownCast(treeNodes->GetPointData()->GetArray(i));
        if(dataArray) {
          auto vecSize = (nodeCorrT.size() == 0 ? dataArray->GetNumberOfValues()
                                                : nodeCorrT.size());
          std::vector<double> vec(vecSize);
          for(unsigned int j = 0; j < vec.size(); ++j) {
            auto toGet = (nodeCorrT.size() == 0 ? j : nodeCorrT[j]);
            vec[j] = dataArray->GetVariantValue(toGet).ToDouble();
          }
  ...
  void makeTreesOutput(std::vector<FTMTree_MT *> &trees,
      ...
      for(int i = 0; i < numInputs; ++i) {
        ...
        while(!queue.empty()) {
            ...
            if(!trees[i]->isRoot(node)) {
              ...
              for(int toAddT = 0; toAddT < toAdd; ++toAddT) {
                ...
                isImportant = trees[i]->isImportantPair<dataType>(
                  ...
                  excludeImportantPairsLower_, excludeImportantPairsHigher_,
                  excludeImportantPairsLower2_, excludeImportantPairsHigher2_);
                ...
                // TODO seems to not work correctly for arc field
          ...
          for(int toAddT = 0; toAddT < toAdd; ++toAddT) {
            ...
            bool isImportant = false;
            isImportant = trees[i]->isImportantPair<dataType>(
              node, importantPairs_, excludeImportantPairsLower_,
              excludeImportantPairsHigher_, excludeImportantPairsLower2_,
              excludeImportantPairsHigher2_);
            ...
            birthArray->InsertNextTuple1(std::get<0>(birthDeath));
            deathArray->InsertNextTuple1(std::get<1>(birthDeath));
        ...
        /*if(OutputSegmentation and not PlanarLayout) {
          auto iVtkUnstructuredGrid
            = vtkUnstructuredGrid::SafeDownCast(treesSegmentation[i]);
          if(not iVtkUnstructuredGrid
             or not iVtkUnstructuredGrid->GetPoints()) {
            std::cout << "ImageData" << std::endl;
            auto iVtkImageData
              = vtkImageData::SafeDownCast(treesSegmentation[i]);
            vtkNew<vtkImageData> iVtkImageDataOutput{};
            iVtkImageDataOutput->DeepCopy(iVtkImageData);
            int x1, x2, y1, y2, z1, z2;
            iVtkImageDataOutput->GetExtent(x1, x2, y1, y2, z1, z2);
            iVtkImageDataOutput->SetExtent(x1 + diff_x, x2 + diff_x,
                                           y1 + diff_y, y2 + diff_y,
                                           z1 + diff_z, z2 + diff_z);
            appendFilter->AddInputData(iVtkImageDataOutput);
          } else {
            vtkNew<vtkUnstructuredGrid> iVtkUnstructuredGridOutput{};
            iVtkUnstructuredGridOutput->DeepCopy(iVtkUnstructuredGrid);
            for(int p = 0;
                p
                < iVtkUnstructuredGridOutput->GetPoints()->GetNumberOfPoints();
                ++p) {
              double *point
                = iVtkUnstructuredGridOutput->GetPoints()->GetPoint(p);
              point[0] += diff_x;
              point[1] += diff_y;
              point[2] += diff_z;
              iVtkUnstructuredGridOutput->GetPoints()->SetPoint(p, point);
            }
            appendFilter->AddInputData(iVtkUnstructuredGridOutput);
          }
        }*/
    ...
    // - Manage node output
    // Custom arrays
    for(unsigned int i = 0; i < customArrays.size(); ++i) {
      vtkNew<vtkDoubleArray> customArrayVtk;
      customArrayVtk->SetName(std::get<0>(customArrays[i]).c_str());
      customArrayVtk->SetNumberOfTuples(customArraysValues[i].size());
      for(unsigned int j = 0; j < customArraysValues[i].size(); ++j)
        customArrayVtk->SetTuple1(j, customArraysValues[i][j]);
      vtkOutputNode->GetPointData()->AddArray(customArrayVtk);
    }
    for(unsigned int i = 0; i < customIntArrays.size(); ++i) {
      vtkNew<vtkIntArray> customArrayVtk;
      customArrayVtk->SetName(std::get<0>(customIntArrays[i]).c_str());
      customArrayVtk->SetNumberOfTuples(customIntArraysValues[i].size());
      for(unsigned int j = 0; j < customIntArraysValues[i].size(); ++j)
        customArrayVtk->SetTuple1(j, customIntArraysValues[i][j]);
      vtkOutputNode->GetPointData()->AddArray(customArrayVtk);
    }
    ...
    if(isPersistenceDiagram) {
      vtkOutputNode->GetPointData()->AddArray(birthArray);
      vtkOutputNode->GetPointData()->AddArray(deathArray);
      vtkOutputNode->GetPointData()->AddArray(isMinMaxPairArray);
    ...
    for(unsigned int i = 0; i < customArrays.size(); ++i) {
      vtkNew<vtkDoubleArray> customArrayVtk;
      customArrayVtk->SetName(std::get<0>(customArrays[i]).c_str());
      customArrayVtk->SetNumberOfTuples(customCellArraysValues[i].size());
      for(unsigned int j = 0; j < customCellArraysValues[i].size(); ++j)
        customArrayVtk->SetTuple1(j, customCellArraysValues[i][j]);
      vtkArcs->GetCellData()->AddArray(customArrayVtk);
    }
    for(unsigned int i = 0; i < customIntArrays.size(); ++i) {
      vtkNew<vtkIntArray> customArrayVtk;
      customArrayVtk->SetName(std::get<0>(customIntArrays[i]).c_str());
      customArrayVtk->SetNumberOfTuples(customCellIntArraysValues[i].size());
      for(unsigned int j = 0; j < customCellIntArraysValues[i].size(); ++j)
        customArrayVtk->SetTuple1(j, customCellIntArraysValues[i][j]);
      vtkArcs->GetCellData()->AddArray(customArrayVtk);
    }
