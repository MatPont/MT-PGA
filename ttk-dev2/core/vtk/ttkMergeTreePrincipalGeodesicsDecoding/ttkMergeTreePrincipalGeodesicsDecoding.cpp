...
int ttkMergeTreePrincipalGeodesicsDecoding::RequestData(
  ...
  // --- Load blocks
  // ------------------------------------------------------------------------------------
  std::vector<vtkMultiBlockDataSet *> inputBary, inputTrees;
  ...
  if((baryTreeNodes.size() != 0
     ...
     or tableCorrelationMTime != tableCorrelation->GetMTime()
     or (blockInputTrees
  ...
  // Vectors
  // -----------------
  vS_ = std::vector<std::vector<std::vector<double>>>(
    numberOfGeodesics,
    std::vector<std::vector<double>>(baryNoNodes, std::vector<double>(2, 0.0)));
  v2s_ = vS_;
  if(inputBary.size() > 1) {
    trees2Vs_ = std::vector<std::vector<std::vector<double>>>(
      numberOfGeodesics, std::vector<std::vector<double>>(
                           baryNoNodes2, std::vector<double>(2, 0.0)));
    trees2V2s_ = trees2Vs_;
  }
  for(unsigned int h = 0; h < inputBary.size(); ++h) {
    auto &vS = (h == 0 ? vS_ : trees2Vs_);
    auto &v2s = (h == 0 ? v2s_ : trees2V2s_);
    bool secondInput = (h == 1);
    for(unsigned int i = 0; i < numberOfGeodesics; ++i) {
      auto baryNoNodesT = (h == 0 ? baryNoNodes : baryNoNodes2);
      for(unsigned int j = 0; j < baryNoNodesT; ++j)
        for(unsigned int k = 0; k < 2; ++k) {
          std::string name1
            = getTableVectorName(numberOfGeodesics, i, 0, k, secondInput);
          vS[i][j][k] = tableVectors->GetColumnByName(name1.c_str())
                          ->GetVariantValue(j)
                          .ToDouble();
          std::string name2
            = getTableVectorName(numberOfGeodesics, i, 1, k, secondInput);
          v2s[i][j][k] = tableVectors->GetColumnByName(name2.c_str())
                           ->GetVariantValue(j)
                           .ToDouble();
        }
    }
  ...
  branchesCorrelationMatrix_ = std::vector<std::vector<double>>(
    baryNoNodes, std::vector<double>(numberOfGeodesics, 0.0));
  ...
  for(unsigned int j = 0; j < numberOfGeodesics; ++j) {
    ...
    for(unsigned int i = 0; i < tableCorrNoRows; ++i) {
      auto baryNodeId = baryNodeIdArray->GetVariantValue(i).ToInt();
      branchesCorrelationMatrix_[baryNodeId][j]
        = tableCorrelation->GetColumnByName(name.c_str())
            ->GetVariantValue(i)
            .ToDouble();
      persCorrelationMatrix_[baryNodeId][j]
  ...
  auto pointScalarFieldNameArray
    = tableCorrelation->GetColumnByName("PointScalarFieldName");
  if(pointScalarFieldNameArray) {
    baryPointScalarFieldName = std::vector<int>(baryNoNodes);
    for(unsigned int j = 0; j < tableCorrNoRows; ++j) {
      auto baryNodeId = baryNodeIdArray->GetVariantValue(j).ToInt();
      baryPointScalarFieldName[baryNodeId]
        = pointScalarFieldNameArray->GetVariantValue(j).ToInt();
    }
  } else
    baryPointScalarFieldName.clear();
...
int ttkMergeTreePrincipalGeodesicsDecoding::runCompute(
  ...
  isPersistenceDiagram_ = ttk::ftm::constructTrees<dataType>(
    inputBary, baryDTree, baryTreeNodes, baryTreeArcs, baryTreeSegmentation);

  if(OutputInputTrees
     or (ReconstructInputTrees and ComputeReconstructionError)) {
    ttk::ftm::constructTrees<dataType>(inputTrees, inputDTrees, inputTreesNodes,
                                       inputTreesArcs, inputTreesSegmentation);
    if(isPersistenceDiagram_)
      mtsFlattening(inputDTrees);
  }

  /*printMsg(baryDTree[0].tree.printTree().str());
  ttk::ftm::computePersistencePairs<dataType>(&(baryDTree[0].tree));
  printMsg(baryDTree[0].tree.template
  printPairsFromTree<dataType>(false).str());*/

  //------------------------------------------------------------------------------------
  // --- Call base
  //------------------------------------------------------------------------------------
  useDoubleInput_ = (baryDTree.size() > 1);
  bool processFirstInput = (not useDoubleInput_ or not ProcessSecondInput);
  std::string preFix{(processFirstInput ? "" : "Second Input ")};

  // Geodesics distances
  if(geodesicsDistances_.size() == 0) {
    printMsg("computeGeodesicsDistance");
    computeGeodesicsDistance<dataType>(baryDTree);
  }

  // Input Trees
  if(OutputInputTrees) {
    printMsg("processInputTrees");
  ...
  if(ReconstructInputTrees) {
    ...
    if(processFirstInput) {
      std::vector<ttk::ftm::MergeTree<dataType>> reconstructedTreesT;
      reconstruction<dataType>(
        baryDTree[0], inputDTrees, reconstructedTreesT, reconstructionErrors);
      ttk::ftm::mergeTreesTemplateToDouble<dataType>(
        reconstructedTreesT, reconstructedTrees);
    } else {
      std::vector<ttk::ftm::MergeTree<dataType>> reconstructedTreesT;
      std::vector<ttk::ftm::MergeTree<dataType>> inputTrees2;
      reconstruction<dataType>(baryDTree[1], inputTrees2, reconstructedTreesT,
                               reconstructionErrors, true);
      ttk::ftm::mergeTreesTemplateToDouble<dataType>(
        reconstructedTreesT, reconstructedTrees);
    }
  } else {
    reconstructedTrees.clear();
    reconstructedTrees2.clear();
  }
  ...
  if(ConstructGeodesicsTrees) {
    ...
    constructGeodesicsTrees<dataType>(baryDTree[0], geodesicsTreesT);
    ttk::ftm::mergeTreesTemplateToDouble<dataType>(
  ...
  if(ConstructEllipses) {
    ...
    constructGeodesicsEllipses<dataType>(baryDTree[0], geodesicsEllipsesT);
    ttk::ftm::mergeTreesTemplateToDouble<dataType>(
  ...
  if(ConstructRectangle) {
    ...
    constructGeodesicsRectangle<dataType>(
      baryDTree[0], geodesicsRectangleT, RectangleMultiplier);
    ttk::ftm::mergeTreesTemplateToDouble<dataType>(
  ...
  if(ConstructSurface) {
    ...
    if(processFirstInput) {
      ...
      constructGeodesicsSurface<dataType>(baryDTree[0], geodesicsSurfaceT);
      ttk::ftm::mergeTreesTemplateToDouble<dataType>(
        geodesicsSurfaceT, geodesicsSurface);
    } else {
      std::vector<ttk::ftm::MergeTree<dataType>> geodesicsSurfaceT;
      constructGeodesicsSurface<dataType>(
        baryDTree[1], geodesicsSurfaceT, true);
      ttk::ftm::mergeTreesTemplateToDouble<dataType>(
        geodesicsSurfaceT, geodesicsSurface);
    }
  } else {
    geodesicsSurface.clear();
    geodesicsSurface2.clear();
  }
...
int ttkMergeTreePrincipalGeodesicsDecoding::runOutput(
  vtkInformationVector *outputVector,
  std::vector<vtkMultiBlockDataSet *> &ttkNotUsed(inputBary),
  std::vector<vtkMultiBlockDataSet *> &inputTrees) {
  printMsg(ttk::debug::Separator::L2);
  ...
  output->SetNumberOfBlocks(noTreesTotal);
...
#pragma omp parallel for schedule(dynamic) num_threads(this->threadNumber_)
  ...
  for(unsigned int i = 0; i < noTreesTotal; ++i) {
    ...
    if(i < steps[0]) {
      ...
      treeType = 1;
      getGeodesicsMiddle<dataType>(mt, vS_, v2s_, ts);
    } else if(i < steps[3]) {
      // Geodesics Trees
      index = i - steps[2];
    ...
    vtkSmartPointer<vtkUnstructuredGrid> vtkOutputArc
      = vtkSmartPointer<vtkUnstructuredGrid>::New();
    // vtkDataSet* vtkOutputSegmentation = nullptr;
    ...
    if(isInputTree) {
      // TODO switch the two for loops : baryNodePers and baryNodeId does not
      // need to iterate through the number of geodesics g
      for(unsigned int g = 0; g < branchesCorrelationMatrix_[0].size(); ++g) {
        std::vector<double> corr(mt.tree.getNumberOfNodes(), 0.0),
          corrPers = corr, baryNodePers(mt.tree.getNumberOfNodes(), -1);
        std::vector<int> baryNodeID(mt.tree.getNumberOfNodes(), -1),
          pointScalarFieldName = baryNodeID;
        for(unsigned int n = 0; n < branchesCorrelationMatrix_.size(); ++n) {
          if((int)matchingMatrix[n][i] != -1) {
            corr[matchingMatrix[n][i]] = branchesCorrelationMatrix_[n][g];
            corrPers[matchingMatrix[n][i]] = persCorrelationMatrix_[n][g];
            baryNodeID[matchingMatrix[n][i]] = n;
            baryNodePers[matchingMatrix[n][i]]
              = baryMTree[0].tree.template getNodePersistence<double>(n);
            if(baryPointScalarFieldName.size() > 0)
              pointScalarFieldName[matchingMatrix[n][i]]
                = baryPointScalarFieldName[n];
          }
        }
        std::string name
          = getTableCorrelationName(branchesCorrelationMatrix_[0].size(), g);
        visuMaker.addCustomArray(name, corr);
        std::string name2 = getTableCorrelationPersName(
          branchesCorrelationMatrix_[0].size(), g);
        ...
        if(baryPointScalarFieldName.size() > 0) {
          std::string pointName{"PointScalarFieldName"};
          visuMaker.addCustomIntArray(pointName, pointScalarFieldName);
        }
      }
    ...
    unsigned int noBlocks = (isPersistenceDiagram_ ? 1 : 2);
    noBlocks += isInputTreeAndGotMesh;
    vtkBlock->SetNumberOfBlocks(noBlocks);
    vtkBlock->SetBlock(0, vtkOutputNode);
    if(not isPersistenceDiagram_)
      vtkBlock->SetBlock(1, vtkOutputArc);
    if(isInputTreeAndGotMesh)
      vtkBlock->SetBlock(2 - isPersistenceDiagram_, vtkOutputSegmentation);
    ...
    if(noReconst != 0 or noInput != 0) {
      ...
      vtkBlock->GetBlock(0)->GetFieldData()->ShallowCopy(fd);
    }
    ...
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(treeTypeArray);

    vtkNew<vtkIntArray> treeIDArray{};
    treeIDArray->SetName("TreeID");
    treeIDArray->InsertNextTuple1(i);
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(treeIDArray);
    ...
    for(unsigned int j = 0; j < ts.size(); ++j) {
      ...
      vtkBlock->GetBlock(0)->GetFieldData()->AddArray(tArray);
      ...
      vtkBlock->GetBlock(0)->GetFieldData()->AddArray(scaledTArray);
    }
    ...
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(isSurfaceArray);
    ...
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(isBoundaryArray);
    ...
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(BoundaryIDArray);
    ...
    vtkBlock->GetBlock(0)->GetFieldData()->AddArray(reconstructionErrorArray);

    // Add new block
    output->SetBlock(i, vtkBlock);
  }
