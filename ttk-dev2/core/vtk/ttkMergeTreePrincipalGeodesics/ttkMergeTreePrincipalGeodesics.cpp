...
int ttkMergeTreePrincipalGeodesics::runCompute(
  ...
  ttk::ftm::constructTrees<dataType>(inputTrees2, intermediateMTrees2,
...
void ttkMergeTreePrincipalGeodesics::makeBarycenterOutput(
  ...
  vtkSmartPointer<vtkUnstructuredGrid> vtkOutputArcBary
    = vtkSmartPointer<vtkUnstructuredGrid>::New();
  vtkSmartPointer<vtkMultiBlockDataSet> vtkBlockBary
  ...
  visuMakerBary.setIsPersistenceDiagram(not normalizedWasserstein_);
  visuMakerBary.makeTreesOutput<dataType>(&(barycenter.tree));

  // Construct multiblock
  unsigned int noBlocks = (normalizedWasserstein_ ? 2 : 1);
  vtkBlockBary->SetNumberOfBlocks(noBlocks);
  vtkBlockBary->SetBlock(0, vtkOutputNodeBary);
  if(normalizedWasserstein_)
    vtkBlockBary->SetBlock(1, vtkOutputArcBary);

  output_barycenter->SetBlock(blockId, vtkBlockBary);
}
...
int ttkMergeTreePrincipalGeodesics::runOutput(
  ...
  output_barycenter->SetNumberOfBlocks(noBlock);
  ...
  isPersistenceDiagram_ = false; // TODO will be removed with full PD support
  ...
  // Tree matching
  // TODO manage matching when isPersistenceDiagram_
