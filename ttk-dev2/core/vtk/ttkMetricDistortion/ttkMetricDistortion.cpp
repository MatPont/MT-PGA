...
int ttkMetricDistortion::RequestData(vtkInformation *ttkNotUsed(request),
  ...
  if(distanceMatrixVTK and not validDistanceMatrix) {
    ...
    printErr("Got " + std::to_string(distanceMatrixVTK->GetNumberOfColumns())
             + " columns in the matrix and " + std::to_string(noPoints)
  ...
  for(unsigned int i = 0; i < 3; ++i) {
    ...
    surfaceIDistanceArray->SetName((type + "EdgeLengthSurface").c_str());
    surfaceIDistanceArray->SetNumberOfTuples(noPoints);
    vtkNew<vtkDoubleArray> metricIDistanceArray{};
    metricIDistanceArray->SetName((type + "EdgeLengthMetric").c_str());
  ...
  surfaceDistance_Array->SetName("EdgeLengthSurface");
  surfaceDistance_Array->SetNumberOfTuples(noCells);
  vtkNew<vtkDoubleArray> metricDistance_Array{};
  metricDistance_Array->SetName("EdgeLengthMetric");
