...
#pragma omp parallel for schedule(dynamic) num_threads(this->threadNumber_)
      ...
      for(unsigned int i = 0; i < dim; ++i) {
        ...
        if(std::isnan(surfaceCurvature[i]))
          surfaceCurvature[i] = 0;

        if(distanceMatrix.size() != 0) {
          metricCurvature[i] = coef * M_PI - sumAngleMetric;
          // metricCurvature[i] *= std::pow(coef, -1);
          if(std::isnan(metricCurvature[i]))
            metricCurvature[i] = 0;
...
#pragma omp parallel for schedule(dynamic) num_threads(this->threadNumber_)
      ...
      for(unsigned int i = 0; i < dim; ++i) {
        ...
        if(std::isnan(surfaceArea[i]))
          surfaceArea[i] = 0;
        if(distanceMatrix.size() != 0) {
          if(std::isnan(metricArea[i]))
            metricArea[i] = 0;
          ratioArea[i] = metricArea[i] / surfaceArea[i];
          if(std::isnan(ratioArea[i]))
            ratioArea[i] = 0;
        }
      }
    }
