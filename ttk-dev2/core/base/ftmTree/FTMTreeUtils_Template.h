...
namespace ttk {
  namespace ftm {
    ...
    bool FTMTree_MT::isImportantPair(idNode nodeId,
                                     double threshold,
                                     double excludeLower,
                                     double excludeHigher,
                                     double excludeLower2,
                                     double excludeHigher2) {
      ...
      return pers > threshold
             and not(pers > rootPers * excludeLower / 100.0
                     and pers < rootPers * excludeHigher / 100.0)
             and not(pers > rootPers * excludeLower2 / 100.0
                     and pers < rootPers * excludeHigher2 / 100.0);
    }

    template <class dataType>
    bool FTMTree_MT::isImportantPair(idNode nodeId, double threshold) {
      return this->isImportantPair<dataType>(
        nodeId, threshold, 0.0, 0.0, 0.0, 0.0);
    }
