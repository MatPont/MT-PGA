...
namespace ttk {
  ...
  /**
   * The MergeTreePrincipalGeodesics class provides methods to compute TODO
   */
  ...
  protected:
    ...
    // 0 : Double _ 1 : Simple _ 3 : Double triangle inequality _
    // 2 : Simple triangle inequality
    int parallelMode_ = 0;

    // Gradient descent [Seguy and Cuturi, 2015]
    // TODO gradient descent does not work anymore (double input etc.)
    ...
    bool augmentedEnergy_ = true;
    double weightPropCost_ = 100.0;
    double weightOrthoCost_ = 100.0;
    double weightMapCost_ = 100.0;

    // Old/Testing
    bool tggProjectWholeVector_ = true; // if true: barycenter on the PG
    bool tggPaperProjection_ = true;
  ...
  public:
    ...
    ~MergeTreePrincipalGeodesics() = default;
...
#pragma omp declare reduction( \
  ...
  : omp_out = omp_in.bestDistance > omp_out.bestDistance ? omp_in : omp_out)
    ...
    // Costs
    //----------------------------------------------------------------------------
    double orthogonalCost(int geodesicNumber,
                          std::vector<std::vector<std::vector<double>>> &vS,
                          std::vector<std::vector<std::vector<double>>> &v2s,
                          std::vector<std::vector<double>> &v,
                          std::vector<std::vector<double>> &v2) {
      return verifyOrthogonality(geodesicNumber, vS, v2s, v, v2, false);
    }
    ...
    // TODO remove all geodesicNumber parameters
    double projectionCost(int gN,
                          std::vector<std::vector<double>> &v,
                          std::vector<std::vector<double>> &v2,
                          std::vector<std::vector<std::vector<double>>> &vS,
                          std::vector<std::vector<std::vector<double>>> &v2s,
      ...
      return weightPropCost_ * regularizerCost(v, v2)
             + weightOrthoCost_ * orthogonalCost(gN, vS, v2s, v, v2)
...
#pragma omp parallel for schedule(dynamic) \
  num_threads(this->threadNumber_) if(parallelize_)
    ...
    void trueGeneralizedGeodesicProjection(std::vector<std::vector<double>> &v1,
                                           std::vector<std::vector<double>> &v2,
                                           bool wholeVectorProjection = false) {
      if(tggPaperProjection_) {
        ...
        std::vector<std::vector<double>> v1_V1OnV2(v1.size()),
          v2_V2OnV1(v1.size()), v1_V1AndV2(v1.size()), v2_V1AndV2(v1.size());
        double costV1OnV2 = 0.0, costV2OnV1 = 0.0, costV1AndV2 = 0.0;

        if(not wholeVectorProjection)
          // Compute projection of each "sub"vector
          for(unsigned int i = 0; i < v1.size(); ++i) {
            positivelyProportionnalProjection(v1[i], v2[i], v1_V1OnV2[i],
                                              v2_V2OnV1[i], v1_V1AndV2[i],
                                              v2_V1AndV2[i]);
            costV1OnV2 += distanceL2(v1[i], v1_V1OnV2[i]);
            costV2OnV1 += distanceL2(v2[i], v2_V2OnV1[i]);
            costV1AndV2 += distanceL2(v1[i], v1_V1AndV2[i])
                           + distanceL2(v2[i], v2_V1AndV2[i]);
          }
        else {
          // Compute whole vector projection
          std::vector<double> v1_flatten, v2_flatten, v1_V1OnV2_flat,
            v2_V2OnV1_flat, v1_V1AndV2_flat, v2_V1AndV2_flat;
          flatten(v1, v1_flatten);
          flatten(v2, v2_flatten);
          positivelyProportionnalProjection(v1_flatten, v2_flatten,
                                            v1_V1OnV2_flat, v2_V2OnV1_flat,
                                            v1_V1AndV2_flat, v2_V1AndV2_flat);
          costV1OnV2 = distanceL2(v1_flatten, v1_V1OnV2_flat);
          costV2OnV1 = distanceL2(v2_flatten, v2_V2OnV1_flat);
          costV1AndV2 = distanceL2(v1_flatten, v1_V1AndV2_flat)
                        + distanceL2(v2_flatten, v2_V1AndV2_flat);
          unflatten(v1_flatten, v1);
          unflatten(v2_flatten, v2);
          unflatten(v1_V1OnV2_flat, v1_V1OnV2);
          unflatten(v2_V2OnV1_flat, v2_V2OnV1);
          unflatten(v1_V1AndV2_flat, v1_V1AndV2);
          unflatten(v2_V1AndV2_flat, v2_V1AndV2);
        }

        // Choose the best one
        /*if(costV1OnV2 <= costV2OnV1 and costV1OnV2 <= costV1AndV2) {
          v1 = v1_V1OnV2;
        } else if(costV2OnV1 <= costV1OnV2 and costV2OnV1 <= costV1AndV2) {
          v2 = v2_V2OnV1;
        } else {
          v1 = v1_V1AndV2;
          v2 = v2_V1AndV2;
    ...
    bool projectionStepConvergence(
      ...
      // Cost 2
      double cost2 = projectionCost(geodesicNumber, v, v2, vS, v2s, optMapCost);
        ...
        if(not useGradientDescent_) {
          ...
          if(useSecondInput)
            trueGeneralizedGeodesicProjection(
              vConcat, v2Concat, tggProjectWholeVector_);
          else
            trueGeneralizedGeodesicProjection(v, v2, tggProjectWholeVector_);
...
#pragma omp task firstprivate(i)
            {
              ...
              ss << i << " _ noSkipped = " << noSkipped << " / " << k_
                 << std::endl;
              ss << omp_get_num_threads() << std::endl;
              std::cout << ss.str();
#ifdef TTK_ENABLE_OPENMP
            } // pragma omp task
#endif
...
#pragma omp taskwait
    ...
    //----------------------------------------------------------------------------
    template <class dataType>
    bool convergenceStep(int geodesicNumber,
                         std::vector<double> &distances,
                         std::vector<std::vector<double>> &v,
                         std::vector<std::vector<double>> &v2,
      ...
      ssReg << "Prop. cost  = " << reg;
      printMsg(ssReg.str());
      if(useGradientDescent_ or augmentedEnergy_)
        frechetEnergy += reg;

      // Ortho. cost
      std::stringstream ssOrthoCost;
      auto orthoCost
        = weightOrthoCost_ * orthogonalCost(geodesicNumber, vS_, v2s_, v, v2);
      ssOrthoCost << "Ortho. cost = " << orthoCost;
      printMsg(ssOrthoCost.str());
      if(augmentedEnergy_)
        frechetEnergy += orthoCost;
    ...
    void
      ...
      // ----- Algorithm
      // TODO if augmentedEnergy_ set optMapCost using opt. map. proj
      // (without modifying the vectors)
      double optMapCost = 0.0;
      bool converged = false;
      while(not converged) {
        std::stringstream ss;
        ...
        // --- Convergence
        bool isBest = convergenceStep(geodesicNumber, distances, v, v2,
                                      oldFrechetEnergy, minFrechetEnergy,
                                      cptBlocked, converged, optMapCost);
        if(isBest) {
          Timer t_copy;
          bestV = v;
          bestV2 = v2;
    ...
    void
      ...
      if(not keepState_ or not barycenterWasComputed_) {
        ...
        if(trees2.size() != 0) {
          ...
          computeOneBarycenter<dataType>(trees2, barycenter2, baryMatchings2_,
                                         ...
                                         false);
          if(doubleInputMinMaxPairMode_ == 0)
      ...
      if(not keepState_) {
        ...
        trees2V2s_.clear();
        allTreesTs_.clear();
        srand(deterministic_ ? 7 : time(NULL));
        t_vectorCopy_time_ = 0.0;
    ...
    void execute(std::vector<ftm::MergeTree<dataType>> &trees,
      ...
      if(trees2.size() != 0) {
        bool useMinMaxPair = (doubleInputMinMaxPairMode_ != 0);
        preprocessingTrees<dataType>(trees2, trees2NodeCorr_, useMinMaxPair);
      }
      printMsg(
...
#pragma omp parallel for schedule(dynamic) \
  num_threads(this->threadNumber_) if(parallelize_)
    ...
    double
      verifyOrthogonality(unsigned int geodesicNumber,
                          std::vector<std::vector<std::vector<double>>> &vS,
                          std::vector<std::vector<std::vector<double>>> &v2s,
                          bool doPrint = true);
    double
