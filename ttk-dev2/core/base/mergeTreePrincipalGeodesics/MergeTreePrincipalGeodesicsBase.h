...
/// \date 2021
///
/// This module defines the %MergeTreePrincipalGeodesicsBase class that computes
/// TODO
///
...
namespace ttk {
  ...
  public:
    ...
    ~MergeTreePrincipalGeodesicsBase() = default;
    ...
    void computeOneDistance(
      ...
      if(useDoubleInput and doubleInputMinMaxPairMode_ == 1) {
...
#pragma omp parallel for schedule(dynamic) num_threads(this->threadNumber_) reduction(+:reconstructionError)
      ...
      for(unsigned int i = 0; i < inputTrees.size(); ++i) {
        ...
        getMultiInterpolation<dataType>(
          barycenter, vS, v2s, allTreesTs[i], reconstructedTree);
        dataType error;
        computeOneDistance<dataType>(
          reconstructedTree, inputTrees[i], error, true);
    ...
    void getInterpolationVector(ftm::MergeTree<dataType> &barycenter,
      ...
      std::vector<std::vector<double>> vNew = v, v2New = v2;
      ...
      while(!queue.empty()) {
        ...
        // - Verify NaN
        if(isnan(interpolationVector[nodeBirth])
           or isnan(interpolationVector[nodeDeath])) {
          std::cout << ssT.str();
    ...
    void getInterpolation(ftm::MergeTree<dataType> &barycenter,
      ...
      getInterpolationVector<dataType>(barycenter, v, v2, t, newScalarsVector);
    ...
    void
      ...
      getInterpolation<dataType>(
        barycenter, vS[0], v2s[0], ts[0], interpolated);
      for(unsigned int i = 1; i < vS.size(); ++i) {
        ftm::MergeTree<dataType> newInterpolated;
        getInterpolation<dataType>(
          interpolated, vS[i], v2s[i], ts[i], newInterpolated);
