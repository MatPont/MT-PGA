...
namespace ttk {
  ...
  protected:
    ...
    double entropicRegularization_ = 1.0;
    ...
    bool normalizeMinMaxPair_ = false;
    ...
    unsigned int doubleInputMinMaxPairMode_ = 0;
  ...
  public:
    ...
    void setEntropicRegularization(double lambda) {
      entropicRegularization_ = lambda;
    }
    ...
    void setNormalizeMinMaxPair(bool normalize) {
      normalizeMinMaxPair_ = normalize;
    }
    ...
    double mixDistancesMinMaxPairWeight(bool isFirstInput) {
      return (
        mixtureCoefficient_ == 0.0 or mixtureCoefficient_ == 1.0
          ...
          : 0.5 / mixDistancesWeight(isFirstInput));
    }

    double mixDistancesWeight(bool isFirstDistance) {
      return (isFirstDistance ? std::min(mixtureCoefficient_ * 2, 1.0)
                              : std::min(-mixtureCoefficient_ * 2 + 2, 1.0));
    }

    template <class dataType>
    double mixDistances(dataType distance1, dataType distance2) {
      /*return mixtureCoefficient_ * distance1
             + (1 - mixtureCoefficient_) * distance2;*/
      return std::sqrt(mixDistancesWeight(true) * distance1 * distance1
                       + mixDistancesWeight(false) * distance2 * distance2);
    }
    ...
    ftm::FTMTree_MT *preprocessingPipeline(ftm::MergeTree<dataType> &mTree,
      ...
      if(not useMinMaxPairT
         and (not branchDecompositionT or doubleInputMinMaxPairMode_ == 0))
    ...
    void branchDecompositionToTree(ftm::FTMTree_MT *tree) {
      /*std::stringstream ssTree, ssPairs;
      ssTree << tree->printTree().str();
      ssPairs << tree->printPairsFromTree<dataType>(true).str();*/
      ...
      for(unsigned int i = 0; i < tree->getNumberOfNodes(); ++i)
        if(tree->getNode(i)->getNumberOfDownSuperArcs() == 1
          ...
          /*printMsg(ssTree.str());
          printMsg(ssPairs.str());
          printMsg(tree->printTree().str());
          printMsg(tree->printPairsFromTree<dataType>(false).str());*/
          std::stringstream ss;
