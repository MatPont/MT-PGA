...
namespace ttk {
  ...
  public:
    ...
    void
      ...
      AssignmentSinkhorn<dataType> solverSinkhorn;
      switch(assignmentSolverID_) {
        ...
        case 2:
          ...
          break;
        case 3:
          solverSinkhorn = AssignmentSinkhorn<dataType>();
          solverSinkhorn.setLambda(entropicRegularization_);
          assignmentSolver = &solverSinkhorn;
          break;
        case 0:
        default:
      ...
      // sinkhornTest(costMatrix, matchings);
    }

    template <class dataType>
    void sinkhornTest(std::vector<std::vector<dataType>> &costMatrix,
                      std::vector<asgnMatchingTuple> &matchings) {
      std::vector<asgnMatchingTuple> matchingsOld = matchings;

      std::cout << "=========================================" << std::endl;

      for(auto matching : matchings)
        std::cout << std::get<0>(matching) << " _ " << std::get<1>(matching)
                  << " _ " << std::get<2>(matching) << std::endl;

      AssignmentSinkhorn<dataType> solverSinkhorn
        = AssignmentSinkhorn<dataType>();
      solverSinkhorn.setInput(costMatrix);
      solverSinkhorn.setBalanced(false);
      solverSinkhorn.setDebugLevel(10);
      solverSinkhorn.run(matchings);

      for(auto matching : matchings)
        std::cout << std::get<0>(matching) << " _ " << std::get<1>(matching)
                  << " _ " << std::get<2>(matching) << std::endl;

      matchings = matchingsOld;
    }
