...
namespace ttk {
  ...
  dataType getMinMaxLocal(ftm::FTMTree_MT *tree,
    ...
    if(tree->isParentInconsistent<dataType>(nodeId)) {
      ...
      // std::cout << "[ERROR] inconsistency" << std::endl;
      // if(tree->getNumberOfNodes() < 1000)
      // std::cout << tree->printTree().str();
      // printPairsFromTree<dataType>(tree, true);
      // std::cout << tree->printNode2<dataType>(nodeId).str();
      // std::cout << tree->printNode2<dataType>(nodeIdParent).str();
    }
