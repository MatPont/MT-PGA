...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("MergeTreePrincipalGeodesicsCurvature");

  // ---------------------------------------------------------------------------
  // Initialize ttkMergeTreePrincipalGeodesicsCurvature module (adjust
  // parameters)
  // ---------------------------------------------------------------------------
  auto mergeTreePrincipalGeodesicsCurvature
    = vtkSmartPointer<ttkMergeTreePrincipalGeodesicsCurvature>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // mergeTreePrincipalGeodesicsCurvature->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkMergeTreePrincipalGeodesicsCurvature filter
      mergeTreePrincipalGeodesicsCurvature->SetInputDataObject(
        i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    mergeTreePrincipalGeodesicsCurvature->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkMergeTreePrincipalGeodesicsCurvature filter
  // ---------------------------------------------------------------------------
  mergeTreePrincipalGeodesicsCurvature->Update();
  ...
  if(!outputPathPrefix.empty()) {
    for(int i = 0;
        i < mergeTreePrincipalGeodesicsCurvature->GetNumberOfOutputPorts();
        i++) {
      auto output
        = mergeTreePrincipalGeodesicsCurvature->GetOutputDataObject(i);
      auto writer
